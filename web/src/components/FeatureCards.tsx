...
const SURFACES = [
  {
    ...
    body: "The router verifies the payment authorisation locally, then hands the transfer to KeeperHub to broadcast. Nothing reaches the chain outside it.",
    tone: "pay",
  },
  {
    kicker: "Webhook workflow",
    title: "Payout workflow",
    body: "Trigger → Check Treasury Balance → Solvency Gate → Pay Provider. Both the check and the transfer live in KeeperHub, not in application code.",
    tone: "pay",
  },
  {
    kicker: "Block trigger",
    title: "Solvency watchdog",
    body: "Check Treasury Balance → Read USDC Decimals → Condition, on every block — whether or not the router is up.",
    tone: "cap",
  },
  {
    kicker: "MCP · kh_ header auth",
    title: "Treasurer agent",
    body: "execute_workflow and get_execution over MCP. A real Claude Code agent runs the payouts and reads back the executions.",
    tone: "cap",
  },
] as const;
