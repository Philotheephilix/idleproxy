    ...
    this.db
      .prepare(
        `INSERT INTO nodes (id, provider_id, adapter, pubkey, status, daily_usd_cap_micros, daily_request_cap, max_concurrency, reserve_fraction, last_heartbeat_at, created_at)
         ...
         ON CONFLICT(id) DO UPDATE SET status = 'online', provider_id = excluded.provider_id, adapter = excluded.adapter, pubkey = excluded.pubkey, last_heartbeat_at = excluded.last_heartbeat_at`,
      )
