<!-- source: https://docs.keeperhub.com/ai-tools/agentic-wallet -->
...
## KeeperHub agentic wallet[](https://docs.keeperhub.com/ai-tools/agentic-wallet#keeperhub-agentic-wallet)
...
### Install[](https://docs.keeperhub.com/ai-tools/agentic-wallet#install)
...
### First payment[](https://docs.keeperhub.com/ai-tools/agentic-wallet#first-payment)

The wallet handles payment; the agent still needs a way to discover and call KeeperHub workflows. That comes from the [KeeperHub MCP server](https://docs.keeperhub.com/ai-tools/mcp-server), which exposes the `search_workflows` and `call_workflow` meta-tools to your agent. You can install the MCP server on its own (see the [MCP server](https://docs.keeperhub.com/ai-tools/mcp-server) page) or bundled with the [KeeperHub Claude Code plugin](https://docs.keeperhub.com/ai-tools/claude-code-plugin), which wires both the MCP server and (soon) the wallet skill in one step.
...
### Safety hooks[](https://docs.keeperhub.com/ai-tools/agentic-wallet#safety-hooks)
...
### Server-side hard limits[](https://docs.keeperhub.com/ai-tools/agentic-wallet#server-side-hard-limits)
...
### Default safety config[](https://docs.keeperhub.com/ai-tools/agentic-wallet#default-safety-config)
...
`allowlisted_contracts` in `safety.json` is a client-side first-pass filter — the hook rejects signing calls whose target contract is not in this list. You can **narrow** it further (for example, remove Tempo USDC.e if your agent only pays on Base). You cannot **widen** it: adding a third contract here has no effect because the [server-side hard limits](https://docs.keeperhub.com/ai-tools/agentic-wallet#server-side-hard-limits) still restrict every signature to Base USDC + Tempo USDC.e. For access to other contracts, contact KeeperHub support so a sub-organisation with a different server-side allowlist can be provisioned.

## Alternatives[](https://docs.keeperhub.com/ai-tools/agentic-wallet#alternatives)

### agentcash[](https://docs.keeperhub.com/ai-tools/agentic-wallet#agentcash)
...
### Coinbase agentic wallet skills[](https://docs.keeperhub.com/ai-tools/agentic-wallet#coinbase-agentic-wallet-skills)
...
## Comparison[](https://docs.keeperhub.com/ai-tools/agentic-wallet#comparison)
...
## Choosing a wallet[](https://docs.keeperhub.com/ai-tools/agentic-wallet#choosing-a-wallet)
...
The KeeperHub agentic wallet is a managed service: KeeperHub runs the Turnkey sub-organisation and proxies signing. You trust KeeperHub to honour the [server-side hard limits](https://docs.keeperhub.com/ai-tools/agentic-wallet#server-side-hard-limits) and the `PreToolUse` hook decision. In return you get no-plaintext-key storage, a three-tier safety hook out of the box, and zero-registration onboarding.
...
## What KeeperHub exposes to the agent[](https://docs.keeperhub.com/ai-tools/agentic-wallet#what-keeperhub-exposes-to-the-agent)
...
## Paying for calls[](https://docs.keeperhub.com/ai-tools/agentic-wallet#paying-for-calls)

Paid workflows settle in USDC on Base (via x402) or USDC.e on Tempo (via MPP). Most workflows cost under `$0.05` per call. See [Marketplace](https://docs.keeperhub.com/workflows/marketplace) for the creator-side view of the same settlement.

## Known limitations[](https://docs.keeperhub.com/ai-tools/agentic-wallet#known-limitations)
...
## FAQ[](https://docs.keeperhub.com/ai-tools/agentic-wallet#faq)

### Who controls my wallet?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#who-controls-my-wallet)
...
### What stops KeeperHub signing whatever it wants?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#what-stops-keeperhub-signing-whatever-it-wants)

A set of Turnkey policies, applied per sub-organisation at provision time and enforced by Turnkey itself (not by application code). Full list above under [Server-side hard limits](https://docs.keeperhub.com/ai-tools/agentic-wallet#server-side-hard-limits). Briefly: signing only against the Base USDC / Tempo USDC.e contracts, no `approve()` above 100 USDC, no `transfer()` or `transferFrom()` above 100 USDC, and EIP-712 signing restricted to allowlisted chain ids and verifying contracts.
...
### What happens if I lose `wallet.json`?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#what-happens-if-i-lose-walletjson)
...
### Can I move the wallet to another machine?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#can-i-move-the-wallet-to-another-machine)
...
### Does KeeperHub have access to my funds?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#does-keeperhub-have-access-to-my-funds)

KeeperHub can produce signatures for your wallet, but only within the limits of the [server-side hard limits](https://docs.keeperhub.com/ai-tools/agentic-wallet#server-side-hard-limits). KeeperHub never sees a private key — the key material lives inside Turnkey’s secure enclave, and Turnkey is the one that produces signatures after KeeperHub’s API key passes the policy engine.

### Why don’t I have a passkey or 2FA option?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#why-dont-i-have-a-passkey-or-2fa-option)
...
### Can I change the safety thresholds or the allowed contracts?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#can-i-change-the-safety-thresholds-or-the-allowed-contracts)
...
Raising thresholds raises your exposure. Widening the contract allowlist past the server-side default (Base USDC + Tempo USDC.e) has no effect on its own — the [server-side hard limits](https://docs.keeperhub.com/ai-tools/agentic-wallet#server-side-hard-limits) still block signatures against any other contract. If you need access to a different contract, contact KeeperHub support.

### How are signing decisions actually enforced?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#how-are-signing-decisions-actually-enforced)
...
2.  **Server-side Turnkey policies**, enforced inside Turnkey for every signing activity. See [Server-side hard limits](https://docs.keeperhub.com/ai-tools/agentic-wallet#server-side-hard-limits) for the full list. They are the hard floor — a misconfigured hook or a compromised agent still cannot sign outside them.
...
### What’s the difference between my wallet and my KeeperHub creator wallet?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#whats-the-difference-between-my-wallet-and-my-keeperhub-creator-wallet)
...
### Can I delete my wallet?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#can-i-delete-my-wallet)
...
### What do I actually pay? Do I need ETH for gas?[](https://docs.keeperhub.com/ai-tools/agentic-wallet#what-do-i-actually-pay-do-i-need-eth-for-gas)
...
## Links[](https://docs.keeperhub.com/ai-tools/agentic-wallet#links)
